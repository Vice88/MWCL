...
%DIF DEL mwcl_riemann_pump_main_01_submitted.tex   Mon Feb 13 17:05:33 2017
%DIF ADD mwcl_riemann_pump_main.tex                Tue Feb 14 17:41:42 2017
...
Further, the \gls{gan} \gls{hemt} technology provides high-switching frequencies\DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{, up to $\text{f}_{\text{t}} = \text{30 GHz}$ for a gate length of 250 nm, to }\DIFaddend ensure an oversampling ratio of 5 for a wide baseband bandwidth. %signal.
...
%The utilization of \gls{gan} enables to design a one-chip solution of a \gls{rfdac} co-integrated with a \gls{pa}, 
...
The \DIFaddbegin \DIFadd{advantage of \mbox{%DIFAUXCMD
\gls{gan} }%DIFAUXCMD
in contrast to 65nm CMOS switches used in \mbox{%DIFAUXCMD
\cite{VeyracRivetDevalEtAl2016} }%DIFAUXCMD
is to switch higher power up to 30 GHz.
%DIF >  up to 6 GHz baseband frequency bands.
The }\DIFaddend possibility to cover a \DIFaddbegin \DIFadd{baseband }\DIFaddend frequency range from DC to 6 GHz enables this architecture to operate with all common mobile communication standards.
...
\DIFdelbegin \DIFdel{The advantage of \mbox{%DIFAUXCMD
\gls{gan} }%DIFAUXCMD
is to switch high power at high frequencies as compared to conventional CMOS switches used in \mbox{%DIFAUXCMD
\cite{VeyracRivetDevalEtAl2016}}%DIFAUXCMD
, which are limited in power.
}\DIFdelend %DIF >   silicon technology at mobile communication frequencies. are limited in power.
...
In contrast to the \DIFdelbegin \DIFdel{CMOS approach in \mbox{%DIFAUXCMD
\cite{VeyracRivetDevalEtAl2016} }%DIFAUXCMD
where simulations, a CMOS }\DIFdelend \DIFaddbegin \DIFadd{simulations, the }\DIFaddend layout and post-layout simulations \DIFdelbegin \DIFdel{are carried out}\DIFdelend \DIFaddbegin \DIFadd{of the CMOS approach in \mbox{%DIFAUXCMD
\cite{VeyracRivetDevalEtAl2016}}%DIFAUXCMD
}\DIFaddend , this paper verifies the concept with measurement results.%DIF >  where simulations, a CMOS layout and post-layout simulations are carried out, 
The presented concept \DIFdelbegin \DIFdel{of the Riemann Pump in \mbox{%DIFAUXCMD
\gls{gan} }%DIFAUXCMD
technology }\DIFdelend %DIF > of the Riemann Pump in \gls{gan} technology
 shows the potential to reach a signal power of up to 20 W while improving the data rate for base station transmitters.
...
%At the end the paper is concluded  
...
% -----------------------------------------------------------------------------
%
Figure \ref{fig:schematic_multibit_rp} shows the schematic of the Riemann Pump, where the push-pull stages \DIFdelbegin \DIFdel{are }\DIFdelend cascaded in parallel \DIFaddbegin \DIFadd{determine the resolution of the \mbox{%DIFAUXCMD
\gls{rfdac}}%DIFAUXCMD
}\DIFaddend .
...
The digital driver circuit\DIFaddbegin \DIFadd{, also realized in GaN technology, }\DIFaddend is marked with dashed lines and is \DIFaddbegin \DIFadd{optimized regarding switching speed of the power transistors.
It is }\DIFaddend necessary for each single stage \DIFdelbegin \DIFdel{. %DIF <  cascaded in parallel.
It }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend enables to control the power transistors, which are intended to be voltage-controlled current sources, with a digital bit stream.
A \DIFaddbegin \DIFadd{highly linear }\DIFaddend co-integrated power amplifier, serving as the output load, enables to reduce the number of external components in the \gls{rffe}.
The \DIFaddbegin \DIFadd{equivalent }\DIFaddend capacitive input impedance of the power amplifier\DIFdelbegin \DIFdel{makes it suitable for this concept.
}\DIFdelend \DIFaddbegin \DIFadd{, which addresses an output power of 20W, is simulated and yielded $\text{C}_{\text{out}} = \text{20 pF}$.
%DIF >   with S-parameter simulation and calculation of small signal parameter e
%DIF >  makes it suitable for this concept. ($\text{C} = \text{20 pF}$)
}\DIFaddend Based on a charge pump the resulting output voltage at the load is defined as:
%
\begin{align}
...
\DIFdelbegin \DIFdel{As eight different slopes are generated, it corresponds with a three-bit resolution of the \mbox{%DIFAUXCMD
\gls{rfdac} }%DIFAUXCMD
enabling an encoding of each slope with a digital bit stream.
}\DIFdelend %DIF > As eight different slopes are generated, it corresponds with a three-bit resolution of the \gls{rfdac} enabling an encoding of each slope with a digital bit stream.
...
\DIFdelbegin \DIFdel{Mandatory for the correct functioning of the push-pull configuration is a digital driver circuit to ensure a proper switching of the high-side transistors.
To prevent a signal delay caused by this driver circuit, this driver is also used for the low-side transistor.
}\DIFdelend %DIF > Mandatory for the correct functioning of the push-pull configuration is a digital driver circuit to ensure a proper switching of the high-side transistors.
%DIF > To prevent a signal delay caused by this driver circuit, this driver is also used for the low-side transistor.
...
\begin{figure}[htb]
  \centering
	\begin{scriptsize}
  	\def\svgwidth{\columnwidth}
 	\DIFdelbeginFL %DIFDELCMD < \input{graphics/circuit_schematic_layout_ddrixy6.pdf_tex} 
%DIFDELCMD <   	%%%
\DIFdelendFL \DIFaddbeginFL \input{graphics/circuit_schematic_layout_ddrixy6_rev2.pdf_tex} 
  	\DIFaddendFL \caption{(a) Schematic of assembly; the used \glspl{mmic} are highlighted in grey. (b) Assembled demonstrator layout.}
  	\label{fig:assembled_demonstrator}
	\end{scriptsize}
\end{figure}
% -----------------------------------------------------------------------------
%
The \DIFdelbegin \DIFdel{green }\DIFdelend \DIFaddbegin \DIFadd{dashed }\DIFaddend rectangles represent off-chip bypass capacitors, the \DIFdelbegin \DIFdel{black }\DIFdelend \DIFaddbegin \DIFadd{solid }\DIFaddend rectangles are the \glspl{mmic} used.
Each \gls{mmic}, consisting of the grey highlighted circuits in Figure \ref{fig:assembled_demonstrator} (a), have got a via to the backside at the power transistors source potential.
Due to the \DIFdelbegin \DIFdel{microstrip approach for the \mbox{%DIFAUXCMD
\gls{mmic} }%DIFAUXCMD
it is }\DIFdelend \DIFaddbegin \DIFadd{common drain configuration for the high side transistors Q1 and Q3, their source potential is not allowed to be fixed to GND as it is for Q2 and Q4.
Hence it is }\DIFaddend necessary to isolate the \glspl{mmic} \DIFdelbegin \DIFdel{for the high-side transistors Q1 and Q3.
This isolation is realized }\DIFdelend by an isolated pad on the substrate, which induces an additional contribution to the thermal conductivity.
%DIF >  Due to the microstrip approach for the \gls{mmic}
%DIF >  This isolation is realized 
...
These signals have to be amplified by a broadband pre-amplifier and shifted in the DC offset with bias tees to ensure proper switching of the transistors Qb1, Qb2, Qb3 and Qb4 at the input. 
\DIFdelbegin \DIFdel{A stability analysis, where the biasing is checked to stay constant, is performed to ensure that the \mbox{%DIFAUXCMD
\gls{dut} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{After the transient time of applying the biasing, a constant power consumption indicated a stable \mbox{%DIFAUXCMD
\gls{dut} }%DIFAUXCMD
that }\DIFaddend does not oscillate.
%DIF > A stability analysis, where the biasing is checked to stay constant, is performed to ensure that the \gls{dut} does not oscillate.
...
To show the feasibility to synthesize different output signals it is necessary to generate different currents
\DIFdelbegin \DIFdel{, which are integrated by a capacitor, }\DIFdelend %DIF > , which are integrated by a capacitor, 
as mentioned in Chapter \ref{sec:theory}.
...
In-phase switching of both \DIFdelbegin \DIFdel{high-side }\DIFdelend \DIFaddbegin \DIFadd{high- }\DIFaddend and low-side transistors led to the results shown in Figure \ref{fig:meas_Output_CLoad_100M_1io_3io} (b).
...
In order to select the slope $1 i_0$ both \DIFdelbegin \DIFdel{high-side }\DIFdelend \DIFaddbegin \DIFadd{high- }\DIFaddend as well as both low-side transistors are driven with anti-phase signals.
...
The \DIFdelbegin \DIFdel{measurement results }\DIFdelend \DIFaddbegin \DIFadd{integration of alternating current to charge and discharge a capacitance leads to the theoretical triangular voltage signal (dashed) }\DIFaddend in Figure \ref{fig:meas_Output_CLoad_100M_1io_3io}\DIFdelbegin \DIFdel{present the output of the digital-to-analog conversion, }\DIFdelend \DIFaddbegin \DIFadd{.
Due to the hybrid assembly and the induced parasitics in addition to the non-ideal switching, a phase drift and harmonic distortion is observed.
Nevertheless, the measurement presents the first results, }\DIFaddend which could drive a co-integrated \gls{pa} to gain a power of 20 W.
%, while a co-integrated \gls{pa} is capable to boost this signal to the wanted power of a transmitter architecture in a base station.
%This output signal can drive the input of a power amplifier, which has a gate periphery of  in the in-house technology  to reach the desired 20 W of power.
%%The measurement yields an output power of 1.38 W for Vdd = 5V although this 
%which shows the possibility to increase the output power to several watts by applying voltages of up to 28V for Vdd.
...
\DIFdelbegin \DIFdel{The measurement results show the feasibility of synthesizing arbitrary waveforms at 100 MHz.
}\DIFdelend %DIF > The measurement results show the feasibility of synthesizing arbitrary waveforms at 100 MHz.
...
A co-integration of a \gls{pa} next to the \gls{rfdac} core is planned to provide high power signals suitable for the transmitter architecture of base stations.
% need for a transmitter architecture in a base station. 
%A next step is to co-implement a broadband power amplifier which is capable to provide output power of up to 20W.
