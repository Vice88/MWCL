...
A two-bit \gls{rfdac} designed with multiple \glspl{mmic} proves the feasibility to generate arbitrary waveforms.
...
One concept that improves the performance is the Riemann Pump \cite{VeyracRivetDevalEtAl2014}, \cite{DevalRivetVeyrac2015}.
...
The advantage of \gls{gan} is to switch high power at high frequencies compared to conventional CMOS switches, which are limited in power.
As the power consumption of \gls{gan} \glspl{hemt} exceeds the limit for mobile devices, the presented architecture is suitable for base station transmitters.\\
...
For the digital switching of the \gls{gan} power transistors a \gls{mmic} is used that has an implemented driver circuit \cite{MaroldtBruecknerQuayEtAl2014}.
...
The potential of the chosen technology promises to cover even higher frequencies, for \gls{mmic} integration of the concept, to satisfy the condition for the new mobile communication standards.
%The next step is to integrate the presented hybrid assembly into a \gls{mmic}.
